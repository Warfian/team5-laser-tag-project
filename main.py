...
def show_player_entry():
    with dpg.window(tag="team_window", label="Teams",no_title_bar=True, no_move=True, no_resize=True, no_scrollbar=True) as team_window:
                ...
                with dpg.table(header_row=True): # Table column headers
                    ...
                    for i in range(15):
                        with dpg.table_row():
                            ...
                            # __40__ (inline change) adds callback to text box
        ...
        with dpg.group(tag="network_address_group", horizontal=True):
            dpg.add_text("Network Address: ")
            dpg.add_input_text(label="", tag="network_address", default_value="127.0.0.1", callback=network_change_callback, on_enter=True)
        dpg.set_item_pos("network_address_group", [125, 600])
        #
        #
        # end 40's code
